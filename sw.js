const CACHE_NAME = 'pscamera-pwa-v24-shooting-mode-fix-restore-20260610';
...
<script id="pscamera-shooting-mode-fix-v24">
(() => {
  'use strict';
  ...
  function setVersionText() {
    ...
    // 念のため、フッター構造が変わっていても文字だけ置き換える。
    try {
      const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
      const targets = [];
      while (walker.nextNode()) {
        const node = walker.currentNode;
        if (/v1\.(21-image-management|24-shooting-mode-fix|25-rotate-tools|25-upload-orientation-fix|26-capture-landscape-fix)/.test(node.nodeValue)) {
          targets.push(node);
        }
      }
      targets.forEach(node => {
        node.nodeValue = node.nodeValue.replace(/v1\.(21-image-management|24-shooting-mode-fix|25-rotate-tools|25-upload-orientation-fix|26-capture-landscape-fix)/g, FIX_VERSION);
      });
    } catch (_) {}
  }
...
function injectIntoHtml(html) {
  ...
  let nextHtml = html
    .replace(/v1\.21-image-management/g, 'v1.24-shooting-mode-fix')
    .replace(/v1\.25-rotate-tools/g, 'v1.24-shooting-mode-fix')
    .replace(/v1\.25-upload-orientation-fix/g, 'v1.24-shooting-mode-fix')
    .replace(/v1\.26-capture-landscape-fix/g, 'v1.24-shooting-mode-fix');
